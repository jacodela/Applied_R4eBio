...
```{r}
# light lm of flowering time per light condition
...
```{r}
pressure_mod = lm(pressure ~ mass, data = PimaIndiansDiabetes2)
```
...
```{r}
# hist
ggplot(pressure_mod, aes(x = residuals(pressure_mod))) +
  geom_histogram()
...
```{r}
# lm adjusting by age
pressure_adj = lm(pressure ~ age + mass, data = PimaIndiansDiabetes2)
summary(pressure_adj)
```
...
```{r}
weight_vore_aov = aov(bodywt ~ vore, data = msleep)
summary(weight_vore_aov)
```

## Exercise 3

Are there differences in the awake times of animals belongind to different orders?

```{r}
awake_order_aov = aov(awake ~ order, data = msleep)
summary(awake_order_aov)
```
...
```{r}
# Tukey test
Tukey_table = TukeyHSD(awake_order_aov)

# Create table and filter
Tukey_table = as.data.frame(Tukey_table$order) 
Tukey_table[Tukey_table$`p adj` < 0.05,]
...
msleep %>% 
  ggplot(aes(log(bodywt), sleep_total)) + 
  geom_point()
```
...
```{r}
# sleep lm
sleep_lm = lm(sleep_total ~ log(bodywt), data = msleep)
summary(sleep_lm)
plot(sleep_lm)
```
...
```{r}
# vore sleep lm
vore_sleep_lm = lm(sleep_total ~ vore + log(bodywt), data = msleep)
summary(order_sleep_lm)
```
