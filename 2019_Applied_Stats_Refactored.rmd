...
## Set seed 
We will be using simulations and generating random numbers. Setting a seed will make sure that we consisntently get the same values.
```{r}
set.seed(2112) 
```
...
For this section we will use the `diamonds` dataset, which contains information about the price and characteristics of 54.000 diamonds. We can test what is the association between the caratage of a diamond and its value
...
To calculate the correlation between petal and sepal lenghts, all we use the `cor` command:
```{r, echo=TRUE}
...
```{r, echo=TRUE}
cor.test(diamonds$carat, diamonds$price, method = "p")
```
...
We'll start with a very simple linear model that just has a single categorical factor with two levels. Let's say we're predicting flowering times for high and low light conditions in a single *Arabidopsis thaliana* genotype. Average flowring time (`mu`) is 45 days, and is 7 days faster for high light than low light conditions (`effect`). 

You need to represent categorical factors with numbers. The numbers, or **coding** you choose will affect the numbers you get out of the analysis and how you need to interpret them. Here, we will **effect code** the light conditions so that high light are coded as +0.5, and low light are coded as -0.5.

Plants of the same genotype won't always respond exactly the same way. Some might flower a little faster the same light condition than others due to random fluctuations in factors such as temperature, nutrients or other unmeassured factors. So we can add an **error term** to each condition. We can't know how much any specific replicate of the experiment will vary, but we can characterise the distribution of how much replicates differ from average and then sample from this distribution. 
...
*Excercise*: Change the values of `mu` and `effect`, resimulate the data, and re-run the linear model. What happens to the estimates?
...
*Excercise*: What happens to the residuals if you fit a model that ignores trial type (e.g., `lm(Y ~ 1, data = dat)`)?

### Predict New Values

You can use the estimates from your model to predict new data points, given values for the model parameters. For this simple example, we just need to now the light condition to make a prediction.

For high light, you would predict that a new data point would be equal to the intercept estimate plus the trial type estimate multiplied by 0.5 (the effect code for high light).
...
The t-test is just a special, limited example of a general linear model. In this case, we are comparing the differences between the means (a continuous dependent variable) between **two groups** (a categorical independent variable with two levels). 

The idea is simple given what we know about the data (and therefore, the variances) within two groups of samples, how likely is it that the samples were drawn from two populations with the same mean? 
...
*Excercise*: What happens when you use other codings for trial type in the t-test above? Which coding maps onto the results of the t-test best?
...
Just like with the t-test, you can perform the ANOVA using the `aov()` command and a formula. **Note** that there is a related but diifferent command named `anova()`, don't confuse them!
...
**Excercise**: Compare the square root of the F-value of the linear model to the t-value from the t-tests above.
```{r}
...
For the following excercises we will use the `iris` dataset that has measurements of different parts of flowers. (See `?iris` for information about the dataset.)
